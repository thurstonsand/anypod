...
Anypod is a thin Python wrapper around **yt-dlp** that converts any yt-dlp–supported source—**video _or_ audio**—into an RSS feed consumable by podcast players. It runs as a long-lived Docker container and is configured solely through YAML.
...
- **Graceful Degradation** – When a subsystem reaches a hard limit (disk full, upstream API quota, etc.) the service enters a _degraded_ mode: the offending feature pauses, the rest keeps working, and logs/metrics/`/healthz` make the problem obvious.
...
_The `yt_args` field is parsed using shell-like syntax and converted to yt-dlp options dictionary, not passed verbatim._
...
1.  **Channel URLs** (e.g., `https://www.youtube.com/@ChannelName`):
    ...
    - _Future enhancement_: Allow configuration to target other tabs like "Live" or "Shorts".
...
- include global size limit such that entire app doesnt exeed certain size
  ...
  - | Policy                                     | What it does                                                                                                                                                                     | Strengths                                                               | Watch‑outs                                                                                                              |
    | ------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------- |
    | **Min‑floor + global LRU** _(recommended)_ | Add `min_last:` per feed (default = 1). Delete oldest downloads across _all_ feeds until the cap is met **but never go below `min_last` for any feed**.                          | Simple mental model; prevents "rare" shows from disappearing.           | If every feed is at its floor and cap is still breached, system enters "degraded‑full" state and refuses new downloads. |
    | **Time‑window floor**                      | Keep at least _N_ days of history per feed (`min_days:`). Evict globally‑oldest downloads that fall outside each feed's window.                                                  | Users often reason in "last 90 days" rather than episode counts.        | Variable episode sizes make space usage less predictable.                                                               |
    | **Weighted eviction**                      | Allow optional `weight:` per feed; compute _effective LRU‑age_ = `real_age / weight`. Evict by that metric.                                                                      | Lets you bias important feeds without hard floors.                      | Harder to predict which download will vanish next; extra YAML tuning.                                                   |
    ...
    | **Archive tier**                           | Move oldest media to a cheap "cold" volume (e.g., S3/Glacier) instead of deleting, while pruning DB rows locally.                                                                | No data loss; total cap becomes _hot‑tier_ only.                        | Requires new storage backend; retrieval latency for old episodes.                                                       |
